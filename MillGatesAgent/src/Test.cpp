...
//
//	Node * n = new Node(state, PAWN_WHITE, 0);
//	NodeExpander e;
//
//	//Aggiunta di una pedina
//	Action a1(POS_NULL, get3Dcoordinates('a','1'), POS_NULL);
//	e.performAction(n, a1);
//	n->getChildren().get(0)->getState()->toStringToSend(); //Expected W in first pos
//	printf("\n");
//
//	//Spostamento di una pedina
//	state->setPawnAt2D('a', '1', PAWN_WHITE);
//	Action a2(get3Dcoordinates('a','1'), get3Dcoordinates('a','4'), POS_NULL);
//	e.performAction(n, a2);
//	n->getChildren().get(1)->getState()->toStringToSend(); //Expected W in second pos
//	printf("\n");
//
//	//Spostamento di una pedina + eliminazione
//	state->setPawnAt2D('c', '5', PAWN_BLACK);
//	Action a3(get3Dcoordinates('a','1'), get3Dcoordinates('a','4'), get3Dcoordinates('c','5'));
//	e.performAction(n, a3);
//	n->getChildren().get(2)->getState()->toStringToSend(); //Expected W in second pos and no black pawns.
//}
//
...
void testZobristHashing(State * state) {
	...
	State * newState = state->result(action, PAWN_WHITE);
	int hash = hasher->quickHash(state, action, hasher->hash(state), PAWN_WHITE);
	...
	state = newState->result(action2, PAWN_WHITE);
	std::cout << "pedina bianca in 0 0 1 QUICK HASH " << hasher->quickHash(newState, action2, hash, PAWN_WHITE) << "\n";
	std::cout << "pedina bianca in 0 0 1 HASH " << hasher->hash(state) << "\n";
}
